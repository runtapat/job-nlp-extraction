...
st.caption("Job Posting Information Extraction — Regex, Tokenization, Topic Identification, POS & NER")
...
with tab1:
    ...
    if st.button("🔍 ประมวลผล", type="primary"):
        if not text_input.strip():
            ...
            st.subheader("✅ ผลลัพธ์การสกัดข้อมูล")
            st.table(pd.DataFrame(result.items(), columns=["ข้อมูล", "ค่าที่สกัดได้"]))
            ...
            with st.expander("ขั้นตอนที่ 2: Tokenization & Normalization"):
                st.write(f"จำนวนคำหลังตัดคำและลบ Stopwords: {len(tokens)}")
                st.write(tokens)

            with st.expander("ขั้นตอนที่ 3: Topic Identification (คะแนนแต่ละหมวดหมู่)"):
                st.bar_chart(pd.Series(scores))

            with st.expander("ขั้นตอนที่ 4: POS Tagging"):
                if tags:
                    st.dataframe(pd.DataFrame(tags, columns=["คำ", "POS Tag"]))
                else:
...
st.divider()
st.caption("จัดทำเพื่อการศึกษา รายวิชา NLP — เทคนิค Regex, Tokenization, Topic Identification, POS & NER")
